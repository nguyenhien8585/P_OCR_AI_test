...
class SuperGentleImageExtractor:
    ...
    def _calculate_insertion_position(self, figure, lines, fig_index, total_figures):
        ...
        for i, line in enumerate(lines):
            ...
            if ((line_lower.startswith('câu ') or line_lower.startswith('bài ') or line_lower.startswith('question ')) 
...
class EnhancedWordExporter:
    ...
    def create_word_document(latex_content: str, extracted_figures=None, images=None, auto_table_convert=True) -> io.BytesIO:
        try:
            ...
            for line in lines:
                ...
                if (line.lower().startswith('câu ') or line.lower().startswith('bài ')) and any(c.isdigit() for c in line):
    ...
    def _is_potential_table_header(line):
        ...
        if line_stripped.startswith('|') and line_stripped.endswith('|'):
            ...
            if all(re.match(r'^[\s:-]+, part.strip()) for part in content.split('|')):
                return True
...
class EnhancedWordExporter:
    ...
    def _is_potential_table_header(line):
        ...
        return False, api_key) is not None
