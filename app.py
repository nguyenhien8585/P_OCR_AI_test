...
class EnhancedImageExtractor:
    ...
    def __init__(self):
        # Tham số chính
        self.min_area_ratio = 0.005      # 0.5% diện tích ảnh gốc
        self.min_area_abs = 1500         # 1500 pixels
        self.min_width = 60              # 60 pixels
        self.min_height = 60             # 60 pixels
        self.max_figures = 12            # Tối đa 12 ảnh
        self.max_area_ratio = 0.45       # Tối đa 45% diện tích
        
        # Tham số cắt ảnh
        self.smart_padding = 20          # Padding thông minh
        self.quality_threshold = 0.7     # Ngưỡng chất lượng
        self.edge_margin = 0.02          # Margin từ rìa (2%)
        
        # Tham số phân tích
        self.text_ratio_threshold = 0.3  # Ngưỡng tỷ lệ text
        self.line_density_threshold = 0.05  # Ngưỡng mật độ line
        self.confidence_threshold = 75    # Ngưỡng confidence
    ...
    def _calculate_insertion_priority(self, line_content):
        """
        Tính priority cho insertion points
        """
        line_lower = line_content.lower()
        priority = 0
        
        # Highest priority: ends with specific patterns
        if re.search(r'(sau|dưới đây|bên dưới|như hình|theo hình):?\s*$', line_lower):
            priority += 100
        
        # High priority: contains figure references
        if re.search(r'(hình vẽ|biểu đồ|đồ thị|bảng|sơ đồ)', line_lower):
            priority += 80
        
        # Medium priority: question end indicators
        if re.search(r'(xét|tính|tìm|xác định|chọn)', line_lower):
            priority += 40
...
3. **Công thức toán học - GIỮ NGUYÊN ${...}$:**
- Inline: ${x^2 + y^2 = z^2}$
- Display: ${\\frac{a+b}{c-d} = \\sqrt{x^2 + y^2}}$
- Vector: ${\\vec{AB} = (x_2 - x_1, y_2 - y_1)}$
- Matrix: ${\\begin{pmatrix} a & b \\\\ c & d \\end{pmatrix}}$

⚠️ YÊU CẦU CHẶT CHẼ:
- TUYỆT ĐỐI giữ nguyên ${...}$ cho mọi công thức
- TUYỆT ĐỐI sử dụng A), B), C), D) cho trắc nghiệm
- TUYỆT ĐỐI sử dụng a), b), c), d) cho đúng sai
- Bao gồm TẤT CẢ text và công thức từ ảnh
- Giữ đúng thứ tự và cấu trúc bố cục
"""
