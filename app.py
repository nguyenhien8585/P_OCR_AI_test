...
# Cấu hình trang
st.set_page_config(
    page_title="PDF/LaTeX Converter - Enhanced with OCR & Phone Processing",
    page_icon="📝",
    layout="wide",
            ...
            for candidate in high_confidence_candidates:
                try:
                    ...
                    final_figures.append({
                        ...
                        "ocr_boost": candidate.get("ocr_boost", False),
                        "ocr_overlap": candidate.get("ocr_overlap", 0)
                    })
                except Exception as e:
                    if self.debug_mode:
        ...
        try:
            ...
            for i, fig in enumerate(figures):
                try:
                    ...
                    # Simple label với OCR boost info
...
def main():
    try:
        st.markdown('<h1 class="main-header">📝 PDF/LaTeX Converter - Enhanced with OCR & Phone Processing</h1>', unsafe_allow_html=True)
        ...
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 2rem; border-radius: 15px; margin-bottom: 2rem; text-align: center;">
            <h2 style="margin: 0;">⚖️ BALANCED TEXT FILTER + 🤖 GOOGLE OCR + 📱 PHONE PROCESSING + 📄 WORD EXPORT</h2>
            <p style="margin: 1rem 0; font-size: 1.1rem;">✅ OCR figure counting • ✅ Phone image processing • ✅ Word export with images • ✅ Advanced filtering</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with st.sidebar:
            ...
            if enable_google_ocr:
                ...
                if ocr_api_key and ocr_api_key != "sk-...........":
...
"""
        ...
        <div style='text-align: center; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 2rem; border-radius: 15px;'>
            <h3>⚖️ BALANCED TEXT FILTER + 🤖 GOOGLE OCR + 📱 ENHANCED PHONE + 📄 WORD EXPORT</h3>
            ...
            <p><strong>📄 PDF + 🖼️ Ảnh đơn + 📱 Professional phone processing + 🤖 OCR counting + 🎯 Confidence ≥65% + 📄 Smart Word insertion</strong></p>
        </div>
        """, unsafe_allow_html=True)
