...
try:
    ...
    class ConversionHistory:
        @staticmethod
        def add_to_history(*args): pass
        @staticmethod
        def show_history(): pass
...
class WordExporter:
    ...
    def create_word_document(latex_content: str, images: List[Image.Image] = None) -> io.BytesIO:
        ...
        for line in lines:
            if line.strip():
                ...
                latex_patterns = re.findall(r'\$([^$]+)\$', line)
                ...
                if latex_patterns:
                    ...
                    for pattern in latex_patterns:
                        # Tìm vị trí của pattern trong line
                        start_pos = line.find(f'${pattern}$', current_pos)
                        
                        # Thêm text trước công thức
                        if start_pos > current_pos:
                            p.add_run(line[current_pos:start_pos])
                        
                        # Thêm equation
                        WordExporter.add_latex_equation(p, pattern)
                        
                        current_pos = start_pos + len(f'${pattern}$')
        ...
        if images:
            ...
            for i, img in enumerate(images):
                ...
                with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp:
                    img.save(tmp.name)
                    doc.add_picture(tmp.name, width=doc.sections[0].page_width - doc.sections[0].left_margin - doc.sections[0].right_margin)
                    os.unlink(tmp.name)
    ...
    @staticmethod
    def add_latex_equation(paragraph, latex_code: str):
        """Thêm equation vào paragraph"""
        # Tạo equation element
        run = paragraph.add_run()
        
        # Tạo math element (simplified version)
        # Trong thực tế, bạn có thể cần sử dụng thư viện chuyên dụng như python-docx-template
        # hoặc tích hợp với MathType
        math_run = run._element
        math_text = f" [{latex_code}] "  # Placeholder cho equation
        run.text = math_text
